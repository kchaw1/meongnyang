...
$.fn.revolver = function(options) {
	...
	plugin.init = function(){
		...
		target = new Date(plugin.settings.year, plugin.settings.month, plugin.settings.day, plugin.settings.hours, plugin.settings.minutes);
	...
	tick = function(){
		...
		if(hit){
			addObj.html(d + plugin.settings.daysDel + h + plugin.settings.hourDel + m + plugin.settings.minDel + s +  plugin.settings.secDel);
		}else{
